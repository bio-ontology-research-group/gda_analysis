...
class GDADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        axiom_embs = torch.cat([self.genes, rel_expanded, right_expanded], dim=1)
...
class FALCON(torch.nn.Module):
    ...
    def forward_gda(self, data, anon_e_emb):
        ...
        exist = self._logical_exist(rel_fs, right_fs)
...
def gda_compute_metrics(logits, ys, c_dict):
...
def gda_evaluate(model, loader, c_dict, anon_e_emb, device):
    model.eval()
    mr, mrr, mh1, mh3, mh10, mh50, mh100 = 0, 0, 0, 0, 0, 0, 0
    ranks = dict()
    counter = 0
...
if __name__ == '__main__':
    ...
    for i in range(cfg.n_models):
        ...
        mr, mrr, mh1, mh3, mh10, mh50, mh100, mauc = gda_compute_metrics(logits_this_model, ys, c_dict)
        # mr, mrr, mh1, mh3, mh10, mh50, mh100, mauc = gda_evaluate(model, gda_dataloader_test, c_dict, anon_e_emb, device)
        ...
        mr, mrr, mh1, mh3, mh10, mh50, mh100, mauc = gda_compute_metrics(agg_preds_so_far, ys, c_dict)
