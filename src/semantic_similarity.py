...
@ck.option("--ic_measure", "-ic", default="sanchez", help="Information content measure")
@ck.option("--pairwise_measure", "-pw", default="lin", help="Pairwise measure")
...
def main(root_dir, ic_measure, pairwise_measure, groupwise_measure):
    wandb_logger = wandb.init(entity="ferzcam", project="gda_book_chapter", group="semantic_similarity", name=f"{ic_measure}_{pairwise_measure}_{groupwise_measure}")
    ...
    hmd_human = pd.read_csv(os.path.join(root_dir, 'HMD_HumanPhenotype.rpt'), sep='\t', header=None)
    hmd_human.columns = ["HGene", "EntrezID", "MGene", "MGI ID", "MP Phenotypes", "Extra"]

    gene2pheno = dict()
    for gene in eval_genes:
        gene_entrez_id = int(gene.split("/")[-1])
        phenotypes_super_list = hmd_human[hmd_human["EntrezID"] == gene_entrez_id]["MP Phenotypes"].values.tolist()
        all_phenotypes = []
        for i, phenotypes_list in enumerate(phenotypes_super_list):
            if pd.isna(phenotypes_list):
                # logger.warning(f"Gene {gene} has no phenotypes at list {i}")
                continue
            phenotypes = phenotypes_list.split(", ")
            phenotypes = ["http://purl.obolibrary.org/obo/" + p.replace(":", "_") for p in phenotypes]
            phenotypes = [p for p in phenotypes if p in existing_mp_phenotypes]
            all_phenotypes.extend(phenotypes)
        gene2pheno[gene_entrez_id] = all_phenotypes

    test_pairs = axioms_to_pairs(ds.testing)
    test_genes, test_phenotypes = zip(*test_pairs)
    
    logger.info("Obtaining Disease-Phenotype associations from phenotype.hpoa")
    hpoa = pd.read_csv(os.path.join(root_dir, 'phenotype.hpoa'), sep='\t', comment='#')

    disease2pheno = dict()
    for disease in test_phenotypes:
        disease = disease.split("/")[-1].replace("_", ":")
        phenotypes = hpoa[hpoa["database_id"] == disease]["hpo_id"].values.tolist()
        assert phenotypes[0].startswith("HP:")
        phenotypes = ["http://purl.obolibrary.org/obo/" + p.replace(":", "_") for p in phenotypes]
        phenotypes = [p for p in phenotypes if p in existing_hp_phenotypes]
        disease2pheno[disease] = phenotypes
    ...
    goConf = GDataConf(GFormat.RDF_XML, os.path.join(root_dir, "upheno_all.owl"))
